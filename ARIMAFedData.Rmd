...
```{r}
library(readxl)
datafed <- read_excel("c:/Users/anila/OneDrive/Desktop/Forecasting/Final/ARIMA/FedData.xls")
View(datafed)
library(forecast)
```
...
```{r}
R4 <- Arima(RES_D1, order = c(2,0,2)) 
summary(R4)

```
```{r}
R5 <- Arima(RES_D1, order = c(3,0,1)) 
summary(R5)
...
```{r}
plot(forecast(R6,h=8))
forR400 <- forecast(R6, h = 8) 
print(forR400)
```
