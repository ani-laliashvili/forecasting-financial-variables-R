...
```{r}
library(readxl)
data500 <- read_excel("c:/Users/anila/OneDrive/Desktop/Forecasting/Final/ARIMA/500Data.xlsx")
View(data500)
library(forecast)
```
...
```{r}
x <- data500[,3]
y <- ts(data = x, start = c(2011,2), frequency = 4, end = c(2019,4)) # data start in 2011Q2 and end in 2019Q4.
#y 
...
The ADF test says that taking out only 1 unit root is needed to make it stationary.
...
```{r}
R5 <- Arima(RES_mean, order = c(1,0,0)) 
summary(R5)
```

```{r}
R6 <- Arima(RES_mean, order = c(1,0,1)) 
summary(R6)
```

```{r}
R7 <- Arima(RES_mean, order = c(2,0,0)) 
summary(R7)
```
...
```{r}
plot(forecast(R_D1,h=8))
forR400 <- forecast(R_D1, h = 8) 
print(forR400)
```
